...
Step 1 requires Python 3.9+ with Jupyter and the packages `pandas`, `numpy`, `scipy`, `matplotlib`, `seaborn`, `pingouin`, and `revpref`. Step 2 requires Stata 16 SE or later with the community-contributed packages `reghdfe` and `estout` (install via `ssc install reghdfe` and `ssc install estout`).
